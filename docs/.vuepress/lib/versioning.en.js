...
module.exports = {
  ...
  linksFor (url) {
    ...
    versions.forEach(version => {
      let item = { text: version, link: `/en/${version}/${url}` }
      links.push(item)
    })
  ...
  generate (version) {
    ...
    this.info(`Generating new version into 'docs/en/${version}' ...`)

    try {
      fse.copySync(`${path}/en/master`, `${path}/en/${version}`)
